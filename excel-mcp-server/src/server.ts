...
import { logger, HealthChecker } from '@office-mcp/shared'

// 创建 Excel 专用健康检查器
const globalHealthChecker = new HealthChecker('excel-mcp-server')
...
class ExcelMCPServer {
  ...
  private getAllTools() {
    const healthCheckTool = {
      name: 'excelHealthCheck',
      description: '检查 Excel MCP Server 的健康状态，返回服务器运行状态、内存使用等信息',
      inputSchema: {
        type: 'object',
        properties: {},
        required: []
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      logger.info(`列出 ${allTools.length} 个 Excel 工具`)
      return {
  ...
  async start(): Promise<void> {
    ...
    logger.info('Excel MCP Server 已启动')
