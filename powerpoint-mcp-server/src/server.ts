...
class PowerPointMCPServer {
  ...
  private tools = getPowerPointTools()
  ...
  private getAllTools() {
    const healthCheckTool = {
      name: 'pptHealthCheck',
      description: '检查 PowerPoint MCP Server 的健康状态，返回服务器运行状态、内存使用等信息',
      inputSchema: {
        type: 'object',
        properties: {},
        required: []
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      logger.info(`列出 ${allTools.length} 个 PowerPoint 工具`)
      return {
  ...
  async start(): Promise<void> {
    ...
    logger.info('PowerPoint MCP Server 已启动')
