 ...
 * 工具执行结果
 */
export interface IPCToolExecutionResult {
  success: boolean
  data?: any
  error?: string
}
 ...
 * 设置 IPC 配置
 */
export function setIPCConfig(config: Partial<IPCConfig>): void {
  globalIPCConfig = { ...globalIPCConfig, ...config }
}

/**
 * 获取当前 IPC 配置
 */
export function getIPCConfig(): IPCConfig {
  return { ...globalIPCConfig }
}
...
export async function sendIPCCommand(
  ...
  // 重试循环
  for (let attempt = 0; attempt <= ipcConfig.maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        const backoffDelay = calculateBackoff(attempt - 1, ipcConfig.retryDelay)
        logger.info(`[MCP_TOOL_FLOW] 🔄 重试 ${attempt}/${ipcConfig.maxRetries}，等待 ${backoffDelay}ms`, {
          toolName,
          callId
        })
      ...
      logger.warn(`[MCP_TOOL_FLOW] ⚠️ 请求失败 (attempt ${attempt + 1}/${ipcConfig.maxRetries + 1})`, {
        toolName,
        callId,
        error: error.message,
        isTimeout,
      ...
      if (!isRetryable || attempt >= ipcConfig.maxRetries) {
        break
      }
    }
  }
  ...
  // 所有重试都失败
  const duration = Date.now() - startTime
  logger.error(`[MCP_TOOL_FLOW] ❌ 工具执行失败（已重试 ${ipcConfig.maxRetries} 次）`, {
    toolName,
    callId,
    duration: `${duration}ms`,
