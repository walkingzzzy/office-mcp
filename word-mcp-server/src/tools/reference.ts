...
export const wordReferenceTool = createActionTool({
  name: 'word_reference',
...
- insertBibliography: 插入参考文献`,
  ...
  paramRules: {
    insertLink: [required('url', 'string')],
    insertBookmark: [required('name', 'string')],
    insertCrossRef: [required('referenceType', 'string'), required('referenceItem', 'string')],
    insertFootnote: [required('text', 'string')],
    insertEndnote: [required('text', 'string')],
    insertCitation: [required('source', 'object')]
  },
  properties: {
    url: {
      type: 'string',
