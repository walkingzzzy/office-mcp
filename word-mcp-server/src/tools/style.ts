...
export const wordStyleTool = createActionTool({
  name: 'word_style',
...
- copyFormat: 复制格式 (需要 sourceRange, targetRange)`,
  ...
  paramRules: {
    apply: [required('styleName', 'string')],
    create: [required('styleName', 'string')],
    setHeading: [required('level', 'number')],
    applyList: [required('listType', 'string')],
    setLineSpacing: [required('spacing', 'number')],
    setBackgroundColor: [required('color', 'string')],
    applyTheme: [required('themeName', 'string')],
    copyFormat: [required('sourceRange', 'object'), required('targetRange', 'object')]
  },
  properties: {
    paragraphIndex: {
      type: 'number',
